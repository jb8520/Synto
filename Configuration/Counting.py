...
class Counting_Menu_View(discord.ui.View):
    ...
    def Embed(self,interaction):
        ...
        Double_Count=Query(interaction.guild.id,'double_count')
        if Channel_id==0:
    ...
    async def counting_channel(self,interaction:discord.Interaction,button:discord.ui.Button):
        if interaction.user.guild_permissions.administrator:
            ...
            try:
                await interaction.response.send_message(embed=discord.Embed(description=f"The Counting Channel is currently set to {interaction.guild.get_channel(Query(interaction.guild.id,'channel_id')).mention}",colour=0x00F3FF),ephemeral=True,view=View)
            except:
    ...
    async def double_count(self,interaction:discord.Interaction,button:discord.ui.Button):
        if interaction.user.guild_permissions.administrator:
            ...
            await interaction.response.send_message(embed=discord.Embed(description=f"Double Counting is currently set to {Query(interaction.guild.id,'double_count')}",colour=0x00F3FF),ephemeral=True,view=View)
            await View.wait()
            await interaction.message.edit(embed=self.Embed(interaction))
        else:
            await interaction.response.send_message("❌ You need to have the administrator permission to use this button",ephemeral=True)
    ...
    async def information(self,interaction:discord.Interaction,button:discord.ui.Button):
        if interaction.user.guild_permissions.administrator:
            ...
            Embed.add_field(name="Double Count",value="> True/False: whether members should be able to count multiple times in a row. True they can, false they can't.",inline=False)
