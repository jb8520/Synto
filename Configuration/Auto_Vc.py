...
class Auto_Vcs_Menu_View(discord.ui.View):
    ...
    def Embed(self,interaction):
        ...
        Vc_Creator_id=Query(interaction.guild.id,'vc_creator_id',Connection=self.Connection)
        Vc_Category_id=Query(interaction.guild.id,'vc_category_id',Connection=self.Connection)
        Member_Role=Query(interaction.guild.id,'member_role',Connection=self.Connection)
        Bypass_Roles=Query(interaction.guild.id,'bypass_roles',Connection=self.Connection)
        if Vc_Creator_id==0:
            Embed.add_field(name="Auto Vc Creator",value=f"> #channel",inline=False)
        else:
            Embed.add_field(name="Auto Vc Creator",value=f"> {interaction.guild.get_channel(Query(interaction.guild.id,'vc_creator_id',Connection=self.Connection)).mention}",inline=False)
        if Bypass_Roles==0:
            Embed.add_field(name="Moderator Roles",value=f"> @moderator roles",inline=False)
        else:
            Roles=""
            for Role_id in Query(interaction.guild.id,'bypass_roles',Connection=self.Connection):
        ...
        if Vc_Category_id==0:
            ...
            Embed.add_field(name="Auto Vc Category",value=f"> {interaction.guild.get_channel(Query(interaction.guild.id,'vc_category_id',Connection=self.Connection)).mention}",inline=False)
        if Member_Role==0:
            Embed.add_field(name="Member Role",value=f"> @role",inline=False)
        else:
            Embed.add_field(name="Member Role",value=f"> {interaction.guild.get_role(Query(interaction.guild.id,'member_role',Connection=self.Connection)).mention}",inline=False)
        return Embed
    ...
    async def vc_creator(self,interaction:discord.Interaction,button:discord.ui.Button):
        if interaction.user.guild_permissions.administrator:
            ...
            if Vc_Creator_id!=0:
                ...
                Channel="#channel"
            View=Vc_Creator_View(self.Connection)
    ...
    async def bypass_roles(self,interaction:discord.Interaction,button:discord.ui.Button):
        if interaction.user.guild_permissions.administrator:
            ...
            View=Bypass_Roles_View(self.Connection)
    ...
    async def vc_Category(self,interaction:discord.Interaction,button:discord.ui.Button):
        if interaction.user.guild_permissions.administrator:
            ...
            if Vc_Category_id!=0:
                ...
                Category="#category"
            View=Vc_Category_View(self.Connection)
    ...
    async def member_role(self,interaction:discord.Interaction,button:discord.ui.Button):
        if interaction.user.guild_permissions.administrator:
            ...
            if Member_Role!=0:
                ...
                Role="#role"
            View=Member_Role_View(self.Connection)
...
class Vc_Creator_Select(discord.ui.ChannelSelect):
    ...
    async def callback(self,interaction):
        if interaction.user.guild_permissions.administrator:
            Configure(interaction.guild_id,Vc_Creator_id=self.values[0].id,Connection=self.Connection)
...
class Member_Role_Select(discord.ui.RoleSelect):
    ...
    async def callback(self,interaction):
        if interaction.user.guild_permissions.administrator:
            Configure(interaction.guild_id,Member_Role=self.values[0].id,Connection=self.Connection)
...
class Bypass_Roles_Select(discord.ui.RoleSelect):
    ...
    async def callback(self,interaction):
        if interaction.user.guild_permissions.administrator:
            ...
            Roles=Roles[:-2]
            Configure(interaction.guild_id,Bypass_Roles=ids,Connection=self.Connection)
